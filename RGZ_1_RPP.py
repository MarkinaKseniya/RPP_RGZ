...
def get_stocks_by_name(name):
    cursor.execute(
        f"""SELECT stock_name, averages FROM stock
...
def fetch_data(company_symbol):
    url = f"https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=IBM&interval=5min&apikey=A0EMQQ5BCYHRVJ19"
